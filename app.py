...
with tab1:
    ...
    st.info("팁: 필수 상자에 있는 항목의 'X'를 누르면 항목이 빠지고, 차순위 상자 빈칸을 클릭해 다시 넣을 수 있습니다. (드래그 앤 드롭 효과)")
    ...
    # 필수 조건 박스 (가장 중요)
    hard_rules = st.multiselect("필수 조건 (하드 제약 - 1순위. 예외 없음)", all_rules, default=["교과 3연강 절대 금지", "운동장 체육 2학급 이하 제한"])
    
    # 차순위 조건 박스 (필수에 없는 나머지 규칙들)
...
with tab2:
    ...
    with col4:
        ...
        support_text = st.text_area("지원 강사 (3연강 예외 교사)", value="류명현, 체육지원, 영어지원", height=68)
        sort_text = st.text_area("엑셀 출력 나열 순서", value="김영순, 임희우, 김홍섭, 김승미, 오혜빈, 박은영, 강수정, 김영혜, 김나영, 황두환, 김효진, 정현미, 박수경, 서예슬, 박은경, 이상수, 김온유, 제현진, 김재수, 전정화, 최나경, 허수정, 우가윤, 오은정, 황현숙, 강영미, 이연경, 오성환, 이준희, 김명규, 류지영, 우지연, 김진호, 이정빈, 서정수, 이기영, 배미래, 박주현, 김미수, 이아정, 임주헌, 김연지, 체육지원, 영어지원, 류명현", height=100)
